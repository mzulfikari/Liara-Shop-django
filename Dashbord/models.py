...
class Address(models.Model):
    ...
    city = models.CharField(max_length=50, verbose_name="استان")
    province = models.CharField(max_length=50, verbose_name="شهرستان")
    ...
    email = models.EmailField(verbose_name="ایمیل تحویل گیرنده", null=True, blank=True)
...
class Notification(models.Model):
    ...
    def delete_expired_notifications(cls):
        now = timezone.now()
        cls.objects.filter(
            models.Q(expiration_date__isnull=False) & models.Q(expiration_date__lt=now)
        ).delete()
